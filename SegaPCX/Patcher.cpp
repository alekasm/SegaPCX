...
bool Patcher::CreateDetourSection(HANDLE hFile, PEINFO* info)
{
  ...
  {
    printf("Error with IMAGE_DOS_SIGNATURE when creating detour section\n");
    return false;
  }
  ...
  SetFilePointer(hFile, 0, NULL, FILE_BEGIN);
  if (!WriteFile(hFile, pByte, fileSize, &dw, NULL))
  {
    printf("Failed to WriteFile. Error Code: %d\n", GetLastError());
    return false;
  }
  ...
  {
    printf("Failed to CloseHandle. Error code: %d\n", GetLastError());
    return false;
  }
